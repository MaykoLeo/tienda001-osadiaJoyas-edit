...
export function ShippingCalculator() {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const response = await fetch('/api/shipping', {
        ...
        body: JSON.stringify({ postalCodeDestination: inputCode.trim() }),
      });
      ...
      setShippingCost(data.shippingCost, inputCode.trim());
      setShippingInfo({
  ...
  const renderResult = () => {
    ...
    if (status === 'success' && shippingCost !== null) {
      return (
        <div className="mt-2 text-sm flex flex-col gap-1 text-green-600">
          ...
          {shippingInfo?.productName && (
            <p className="text-xs text-muted-foreground pl-6">
              {shippingInfo.productName}
              {shippingInfo.deliveryTimeMin && shippingInfo.deliveryTimeMax && (
                <> · {shippingInfo.deliveryTimeMin} a {shippingInfo.deliveryTimeMax} días hábiles</>
              )}
            </p>
          )}
        </div>
