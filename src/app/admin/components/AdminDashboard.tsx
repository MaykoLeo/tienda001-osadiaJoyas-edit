...
import { RefreshCw, LogOut, Loader2, Mail, Database, HardDrive, ShoppingBag } from 'lucide-react';
import { addProductAction, updateProductAction, deleteProductAction, deleteOrphanedImageAction, deleteProductImageAction, toggleProductFeaturedAction } from '@/app/actions/product-actions';
...
export function AdminDashboard({ onLogout, dbConnected }: { onLogout: () => void, dbConnected: boolean }) {
    ...
    const fetchMetrics = async (startDate?: Date, endDate?: Date) => {
        ...
        try {
            ...
            toast({ title: 'Error al cargar las métricas', description: (error as Error).message, variant: 'destructive' });
        }
    ...
    const handleFormSubmit = async () => {
        ...
        if (!result) {
            ...
            await fetchData(); // Actualizar datos ANTES de cerrar
            handleCloseDialog();
        }
    ...
    const handleToggleFeatured = async (productId: number, featured: boolean) => {
        ...
        if (!result.success) {
            toast({ title: 'Error', description: result.error || 'No se pudo actualizar el producto', variant: 'destructive' });
            // Recargar datos para revertir el cambio en el UI
            await fetchData();
        } else {
            toast({ title: 'Éxito', description: result.message });
    ...
    const exportOrdersToCSV = () => {
        const headers = ['ID de Orden', 'Fecha', 'Cliente', 'Email', 'Total', 'Tipo de Pedido', 'Tipo de Pago', 'Estado', 'Cupón', 'Descuento', 'ID de Pago', 'Dirección de Envío', 'Productos'];
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                <div className='text-center md:text-left space-y-3'>
                    ...
                    <div className="flex justify-center md:justify-start">
                        ...
                        {/* Mailchimp badge hidden as requested */}
                    </div>
                </div>
                ...
                <div className="self-center">
                    <Button
                        size="lg"
                        className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-xl transform hover:scale-105 transition-all text-base px-8 py-6 font-bold"
                        onClick={() => setDialogType('manual_sale')}
                    >
            ...
            <Dialog open={dialogType !== null && dialogType !== 'import'} onOpenChange={(isOpen) => !isOpen && handleCloseDialog()}>
                <DialogContent
                    className={cn("grid-rows-[auto_1fr_auto] max-h-[95vh] flex flex-col transition-all duration-300", dialogType === 'manual_sale' ? "sm:max-w-[1000px] h-[90vh]" : "sm:max-w-[625px]")}
                    onInteractOutside={(e) => {
                        const target = e.target as HTMLElement;
                        if (target.closest('[role="alert"]') || target.closest('[data-radix-toast-viewport]') || target.closest('[toast-close]')) {
                            e.preventDefault();
                        }
                    }}
                >
                    <DialogHeader>
                        <DialogTitle>
                            {dialogType === 'product' ? (editingProduct ? 'Editar Producto' : 'Añadir Nuevo Producto') :
                                dialogType === 'coupon' ? (editingCoupon ? 'Editar Cupón' : 'Crear Nuevo Cupón') :
                                    dialogType === 'manual_sale' ? 'Nueva Orden de Compra (POS)' : ''}
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    {dialogType === 'manual_sale' ? (
                        <div className="flex-1 overflow-hidden">
                            <POSOrderForm
                                ...
                                onSuccess={() => { handleCloseDialog(); fetchData(); }}
                            />
                        </div>
                    ) : (
                        <>
