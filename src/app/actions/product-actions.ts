...
import { createProduct, updateProduct, deleteProduct, getProductById, getFilteredProducts } from '@/lib/data/products';
...
// Nota: Esta función ahora solo se usa para validación en toggleProductFeaturedAction
// El conteo en el frontend se hace directamente desde el estado de productos
export async function getFeaturedProductsCount() {
    try {
        // Usamos getFilteredProducts que es la función principal de consulta
        const products = await getFilteredProducts({});
        return products.filter(p => p.featured).length;
    } catch (e: any) {
        console.error('Error getting featured products count:', e);
        return 0;
    }
}

// Toggle featured status de un producto
export async function toggleProductFeaturedAction(productId: number, featured: boolean) {
    try {
        // Si se intenta marcar como destacado, validar el límite de 8
        if (featured) {
            const currentFeaturedCount = await getFeaturedProductsCount();

            if (currentFeaturedCount >= 8) {
                return {
                    success: false,
                    error: 'Máximo de 8 productos destacados alcanzado. Desmarca otro producto primero.'
                };
            }
        }
        ...
        try {
            // Solo actualizar el campo featured, no todos los campos del producto
            await updateProduct(productId, { featured });
        } catch (updateError: any) {
            console.error('[toggleProductFeaturedAction] Update failed:', updateError);
            throw updateError;
        }
        ...
        return {
            success: true,
            message: featured ? 'Producto marcado como destacado' : 'Producto desmarcado como destacado'
        };
    } catch (e: any) {
        console.error('[toggleProductFeaturedAction] Error:', e);
        return {
            success: false,
