...
// --- FUNCIONES CRUD (CON FILRRADO) ---
export async function getProducts(params: any = {}): Promise<Product[]> {
    let filtered = [...localProducts];
    const { query, category, minPrice, maxPrice, onSale, featured, sortBy, sortDirection } = params;
    ...
    if (category && category !== 'All') {
        ...
        filtered = filtered.filter(p => p.categoryIds.includes(catId));
    }
    ...
    if (onSale) {
        filtered = filtered.filter(p => p.salePrice !== null && p.salePrice < p.price);
    }
    ...
    if (sortBy) {
        filtered.sort((a: any, b: any) => {
            const valA = sortBy === 'price' ? (a.salePrice ?? a.price) : a[sortBy] ?? "";
            const valB = sortBy === 'price' ? (b.salePrice ?? b.price) : b[sortBy] ?? "";
...
export async function getOrders(): Promise<Order[]> {
    if (localOrders.length === 0) {
        ...
        if (p1 && p4 && p3) {
            localOrders = [
                { id: 1, customerFirstName: "Juan", customerLastName: "Pérez", customerEmail: "juan.perez@example.com", customerPhone: "1122334455", total: 67500, status: 'paid', createdAt: new Date('2024-07-20T10:30:00Z'), items: [{ productId: 1, name: p1.name, image: p1.images[0], quantity: 1, priceAtPurchase: 67500, originalPrice: 75000 }], paymentId: '123456789', deliveryMethod: 'shipping', paymentType: 'QR / Tarjeta', shippingAddress: 'Av. Corrientes 123', shippingLocality: 'CABA', shippingPostalCode: '1043' },
                { id: 2, customerFirstName: "Maria", customerLastName: "García", customerEmail: "maria.garcia@example.com", customerPhone: "1122334455", total: 38250, status: 'shipped', createdAt: new Date('2024-07-19T15:00:00Z'), items: [{ productId: 4, name: p4.name, image: p4.images[0], quantity: 1, priceAtPurchase: 38250, originalPrice: 45000 }], paymentId: '987654321', deliveryMethod: 'pickup', paymentType: 'QR / Tarjeta', pickupName: 'Maria García', pickupDni: '12345678' },
                { id: 3, customerFirstName: "Carlos", customerLastName: "López", customerEmail: "carlos.lopez@example.com", customerPhone: "1122334455", total: 120000, status: 'pending_payment', createdAt: new Date('2024-07-21T09:00:00Z'), items: [{ productId: 3, name: p3.name, image: p3.images[0], quantity: 1, priceAtPurchase: 120000, originalPrice: null }], deliveryMethod: 'shipping', paymentType: 'QR / Tarjeta', shippingAddress: 'Av. de Mayo 567', shippingLocality: 'CABA', shippingPostalCode: '1084' },
            ];
            nextOrderId = 4;
        }
    }
